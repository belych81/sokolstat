...
class Nfl 
{
    ...
    public function scoreMatch(array $data)
    {
        if(!empty($data)){
            foreach($data as $arr){
                $sql = "UPDATE matches SET goal1=".$arr['goal1'].", goal2=".$arr['goal2'].", bomb='".$arr['bomb']."' WHERE id=" . $arr['id'];
                var_dump($sql);
                $this->mysqli->query($sql);
            }
        }
    ...
    public function setTable(array $data)
    {var_dump($data);
        if(!empty($data)){
            foreach($data as $arr){
                if($arr['goal1'] > $arr['goal2']){
                    $sql1 = "UPDATE shiptable SET wins=wins+1, mz=mz+".$arr['goal1'].", mp=mp+".$arr['goal2'].", score=score+3 WHERE team_id=" . $arr['team'];
                    $sql2 = "UPDATE shiptable SET porazh=porazh+1, mz=mz+".$arr['goal2'].", mp=mp+".$arr['goal1']." WHERE team_id=" . $arr['team2'];
                } elseif($arr['goal1'] < $arr['goal2']){
                    $sql1 = "UPDATE shiptable SET wins=wins+1, mz=mz+".$arr['goal2'].", mp=mp+".$arr['goal1'].", score=score+3 WHERE team_id=" . $arr['team2'];
                    $sql2 = "UPDATE shiptable SET porazh=porazh+1, mz=mz+".$arr['goal1'].", mp=mp+".$arr['goal2']." WHERE team_id=" . $arr['team'];
                } else {
                    $sql1 = "UPDATE shiptable SET nich=nich+1, mz=mz+".$arr['goal2'].", mp=mp+".$arr['goal1'].", score=score+1 WHERE team_id=" . $arr['team2'];
                    $sql2 = "UPDATE shiptable SET nich=nich+1, mz=mz+".$arr['goal1'].", mp=mp+".$arr['goal2'].", score=score+1 WHERE team_id=" . $arr['team'];
                }
                var_dump($sql1);
                var_dump($sql2);
